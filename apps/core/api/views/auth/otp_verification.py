...
class OTPVerificationView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user.active_session_key:
                try:
                    ...
                    logger.info(f"Deleted previous session for user {user.employee_code}")
            ...
            user.active_session_key = tokens["refresh"][:40]  # Use first 40 chars as session key
            user.save(update_fields=["active_session_key"])
            
            logger.info(f"User {user.employee_code} completed login successfully")
            response_data = {
        ...
        logger.warning(f"Invalid OTP verification attempt: {serializer.errors}")
