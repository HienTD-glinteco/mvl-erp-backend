...
from apps.core.utils.jwt import get_mobile_token_version, set_mobile_token_version
...
class ClientAwareTokenRefreshSerializer(TokenRefreshSerializer):
    ...
    def validate(self, attrs):  # noqa: C901
        ...
        # Use the freshest mobile_token_version when possible to avoid stale cache
        if user is not None:
            current_tv = int(user.mobile_token_version)
            set_mobile_token_version(user_id=user_id, version=current_tv)
        else:
