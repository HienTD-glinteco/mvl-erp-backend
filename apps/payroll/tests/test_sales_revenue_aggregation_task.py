...
class TestSalesRevenueAggregationTask:
    ...
    @pytest.mark.django_db
    def test_aggregate_with_invalid_date_format(self):
        """Task with invalid date format should return error."""
        result = aggregate_sales_revenue_report_task("invalid-date")
...
class TestOnImportComplete:
    ...
    def test_skips_if_no_target_month(self):
        """on_import_complete should NOT call task if no target_month in options."""
        with patch("apps.payroll.import_handlers.sales_revenue.aggregate_sales_revenue_report_task") as mock_task:
            options = {}

            on_import_complete(import_job_id=1, options=options)

            # Task should NOT be called when target_month is missing
            mock_task.delay.assert_not_called()

    def test_skips_if_invalid_target_month_format(self):
        """on_import_complete should NOT call task if target_month format is invalid."""
        with patch("apps.payroll.import_handlers.sales_revenue.aggregate_sales_revenue_report_task") as mock_task:
            options = {"target_month": "invalid"}

            on_import_complete(import_job_id=1, options=options)

            # Task should NOT be called when target_month format is invalid
            mock_task.delay.assert_not_called()
