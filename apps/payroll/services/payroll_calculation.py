...
from apps.hrm.models import Contract, Employee, EmployeeDependent, EmployeeMonthlyTimesheet
...
class PayrollCalculationService:
    ...
    def _get_active_contract(self) -> Optional[Contract]:
        ...
        return Contract.objects.filter(
            ...
            effective_date__lte=self.period.month
    ...
    def _calculate_business_progressive_salary(self):
        ...
        sales_revenue_obj = SalesRevenue.objects.filter(
            employee=self.employee,
        ...
        business_grade, business_progressive_salary = calculate_business_progressive_salary(
            ...
            self.config['business_progressive_salary']['tiers']
        )
    ...
    def _get_timesheet(self) -> Optional[EmployeeMonthlyTimesheet]:
        ...
        return EmployeeMonthlyTimesheet.objects.filter(
            employee=self.employee,
            report_date=self.period.month
        ).first()
    ...
    def _calculate_overtime_pay(self):
        ...
        sunday_pay = (
            ...
            self.slip.hourly_rate * 
            Decimal(str(overtime_multipliers['sunday']))
        )
        
        holiday_pay = (
            self.slip.holiday_overtime_hours * 
            self.slip.hourly_rate * 
            Decimal(str(overtime_multipliers['holiday']))
        )
    ...
    def _calculate_travel_expenses(self):
        ...
        travel_expenses = TravelExpense.objects.filter(
            employee=self.employee,
            month=self.period.month
        )
        
        taxable = travel_expenses.filter(
            expense_type=TravelExpense.ExpenseType.TAXABLE
        ).aggregate(Sum('amount'))['amount__sum'] or 0
        
        non_taxable = travel_expenses.filter(
    ...
    def _calculate_insurance_contributions(self):
        ...
        social_insurance_base = min(
            self.slip.base_salary,
            Decimal(str(si_config['salary_ceiling']))
        )
        ...
        self.slip.employee_health_insurance = (
            hi_base * Decimal(str(hi_config['employee_rate']))
        ).quantize(Decimal('1'))
        self.slip.employer_health_insurance = (
            hi_base * Decimal(str(hi_config['employer_rate']))
        ).quantize(Decimal('1'))
        ...
        self.slip.employee_unemployment_insurance = (
            ui_base * Decimal(str(ui_config['employee_rate']))
        ).quantize(Decimal('1'))
        self.slip.employer_unemployment_insurance = (
            ui_base * Decimal(str(ui_config['employer_rate']))
        ).quantize(Decimal('1'))
        ...
        self.slip.employee_union_fee = (
            uf_base * Decimal(str(uf_config['employee_rate']))
        ).quantize(Decimal('1'))
        self.slip.employer_union_fee = (
            uf_base * Decimal(str(uf_config['employer_rate']))
        ).quantize(Decimal('1'))
        ...
        self.slip.employer_accident_insurance = (
            ai_base * Decimal(str(ai_config['employer_rate']))
        ).quantize(Decimal('1'))
    ...
    def _calculate_personal_income_tax(self):
        ...
        # Get dependent count
        dependent_count = EmployeeDependent.objects.filter(
            employee=self.employee,
            is_active=True
        ).count()
        ...
        self.slip.dependent_deduction = (
            dependent_count * Decimal(str(tax_config['dependent_deduction']))
        )
    ...
    def _process_recovery_vouchers(self):
        ...
        vouchers = RecoveryVoucher.objects.filter(
            employee=self.employee,
            month=self.period.month
        )
        
        back_pay = vouchers.filter(
            voucher_type=RecoveryVoucher.VoucherType.BACK_PAY
        ).aggregate(Sum('amount'))['amount__sum'] or 0
        
        recovery = vouchers.filter(
    ...
    def _update_related_models_status(self):
        ...
        SalesRevenue.objects.filter(
            employee=self.employee,
            month=self.period.month
        ).update(status=SalesRevenue.Status.CALCULATED)
        
        # Update travel expenses status
        TravelExpense.objects.filter(
            employee=self.employee,
            month=self.period.month
        ).update(status=TravelExpense.Status.CALCULATED)
        
        # Update recovery vouchers status
        RecoveryVoucher.objects.filter(
            employee=self.employee,
            month=self.period.month
        ).update(status=RecoveryVoucher.Status.CALCULATED)
