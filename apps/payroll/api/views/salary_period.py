...
class SalaryPeriodViewSet(AuditLoggingMixin, BaseModelViewSet):
    ...
    def payrollslips_export(self, request, pk=None):
        ...
        Query Parameters:
            async (bool): If 'true', export asynchronously via Celery task and return task_id.
                         If 'false' or not provided, export synchronously and return file URL directly.
        """
        period = self.get_object()

        # Check for async parameter
        is_async = request.query_params.get("async", "false").lower() in ("true", "1", "yes")

        if is_async:
            # Async export via Celery task
            from apps.payroll.tasks import export_payroll_slips_task

            task = export_payroll_slips_task.delay(str(period.id))
