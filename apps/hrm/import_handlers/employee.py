...
# Employee type mapping - maps contract type names (lowercase) to EmployeeType values
...
def map_import_contract_type_to_employee_type(
    ...
    Uses hardcoded mapping to translate contract type names to employee_type values.
    ...
    normalized_value = contract_type_value.strip().lower()

    # Look up in hardcoded mapping
    return EMPLOYEE_TYPE_MAPPING.get(normalized_value)
...
def ensure_default_banks() -> dict[str, Bank | None]:
    ...
    """
    vpbank, _ = lookup_or_create_bank("VPBank", "Ngân hàng TMCP Việt Nam Thịnh Vượng")
    vietcombank, _ = lookup_or_create_bank("Vietcombank", "Ngân hàng TMCP Ngoại thương Việt Nam")
...
def _extract_employee_data(  # noqa: C901
    ...
    if start_date:
        ...
        employee_data["start_date"] = timezone.localdate()
    ...
    if status:
        ...
        if status == Employee.Status.RESIGNED:
            employee_data["resignation_start_date"] = timezone.localdate()
