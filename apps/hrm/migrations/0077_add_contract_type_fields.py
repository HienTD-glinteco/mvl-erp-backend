# Generated by Django 5.2.6 on 2025-11-27 03:17
...
def populate_contract_type_codes(apps, schema_editor):
    """Populate code field for existing contract types."""
    ContractType = apps.get_model("hrm", "ContractType")
    for contract_type in ContractType.objects.filter(code__isnull=True):
        # Generate code based on ID
        if contract_type.id < 1000:
            subcode = f"{contract_type.id:03d}"
        else:
            subcode = str(contract_type.id)
        contract_type.code = f"LHD{subcode}"
        contract_type.save(update_fields=["code"])


def reverse_populate_codes(apps, schema_editor):
    """Reverse migration - set codes to None."""
    ContractType = apps.get_model("hrm", "ContractType")
    ContractType.objects.all().update(code=None)
    ...
    operations = [
        ...
        migrations.AddField(
            ...
            field=models.DecimalField(
                decimal_places=0, default=0, help_text="Base salary amount", max_digits=15, verbose_name="Base salary"
            ),
        ),
        migrations.AddField(
            model_name="contracttype",
        ...
        migrations.AddField(
            ...
            field=models.BooleanField(
                default=True, help_text="Whether social insurance is included", verbose_name="Has social insurance"
            ),
        ),
        migrations.AddField(
            model_name="contracttype",
        ...
        migrations.AlterField(
            ...
            name="name",
            field=models.CharField(
                help_text="Name of the contract type", max_length=100, unique=True, verbose_name="Contract type name"
            ),
        ),
        # Data migration to populate codes for existing records
        migrations.RunPython(populate_contract_type_codes, reverse_populate_codes),
    ]
