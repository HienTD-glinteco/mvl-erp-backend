...
class TestAssignToTimesheetEntry:
    """Tests for Proposal._assign_to_timesheet_entry method."""

    def test_creates_junction_when_timesheet_entry_exists(self, db, test_employee):
        """Test that ProposalTimeSheetEntry is created when TimeSheetEntry exists."""
        # Create a timesheet entry for the complaint date
        timesheet_entry = TimeSheetEntry.objects.create(
            employee=test_employee,
            date=date(2025, 11, 10),
        )

        # Create proposal without triggering save (which calls _assign_to_timesheet_entry)
        proposal = Proposal(
            proposal_type=ProposalType.TIMESHEET_ENTRY_COMPLAINT,
            created_by=test_employee,
            timesheet_entry_complaint_complaint_date=date(2025, 11, 10),
            code="DX-ASSIGN-001",
        )
        # Save without calling _assign_to_timesheet_entry by bypassing save()
        Proposal.objects.bulk_create([proposal])
        proposal = Proposal.objects.get(code="DX-ASSIGN-001")

        # Ensure no junction exists yet
        assert ProposalTimeSheetEntry.objects.filter(proposal=proposal).count() == 0

        # Call _assign_to_timesheet_entry
        proposal._assign_to_timesheet_entry()

        # Verify junction was created
        assert ProposalTimeSheetEntry.objects.filter(proposal=proposal).count() == 1
        junction = ProposalTimeSheetEntry.objects.get(proposal=proposal)
        assert junction.timesheet_entry == timesheet_entry

    def test_does_not_create_duplicate_junction(self, db, test_employee):
        """Test that calling _assign_to_timesheet_entry twice does not create duplicate."""
        timesheet_entry = TimeSheetEntry.objects.create(
            employee=test_employee,
        ...
        proposal = Proposal(
            proposal_type=ProposalType.TIMESHEET_ENTRY_COMPLAINT,
            created_by=test_employee,
            timesheet_entry_complaint_complaint_date=date(2025, 11, 11),
            code="DX-ASSIGN-002",
        )
        Proposal.objects.bulk_create([proposal])
        proposal = Proposal.objects.get(code="DX-ASSIGN-002")

        # First call creates the junction
        proposal._assign_to_timesheet_entry()
        assert ProposalTimeSheetEntry.objects.filter(proposal=proposal).count() == 1

        # Second call should not create a duplicate (validation prevents it)
        proposal._assign_to_timesheet_entry()
        assert ProposalTimeSheetEntry.objects.filter(proposal=proposal).count() == 1

    def test_no_junction_when_timesheet_entry_missing(self, db, test_employee):
        """Test that no junction is created when TimeSheetEntry does not exist."""
        proposal = Proposal(
            proposal_type=ProposalType.TIMESHEET_ENTRY_COMPLAINT,
            created_by=test_employee,
            timesheet_entry_complaint_complaint_date=date(2025, 11, 12),
            code="DX-ASSIGN-003",
        )
        Proposal.objects.bulk_create([proposal])
        proposal = Proposal.objects.get(code="DX-ASSIGN-003")

        # No timesheet entry exists for this date
        proposal._assign_to_timesheet_entry()

        # Verify no junction was created
        assert ProposalTimeSheetEntry.objects.filter(proposal=proposal).count() == 0

    def test_no_junction_for_non_complaint_proposal(self, db, test_employee):
        """Test that no junction is created for non-TIMESHEET_ENTRY_COMPLAINT proposals."""
        ...
        proposal = Proposal(
            ...
            created_by=test_employee,
            timesheet_entry_complaint_complaint_date=date(2025, 11, 13),
            code="DX-ASSIGN-004",
        )
        Proposal.objects.bulk_create([proposal])
        proposal = Proposal.objects.get(code="DX-ASSIGN-004")

        proposal._assign_to_timesheet_entry()

        # Verify no junction was created for non-complaint type
        assert ProposalTimeSheetEntry.objects.filter(proposal=proposal).count() == 0

    def test_no_junction_when_complaint_date_missing(self, db, test_employee):
        """Test that no junction is created when complaint_date is None."""
        proposal = Proposal(
            proposal_type=ProposalType.TIMESHEET_ENTRY_COMPLAINT,
            created_by=test_employee,
            timesheet_entry_complaint_complaint_date=None,
            code="DX-ASSIGN-005",
        )
        Proposal.objects.bulk_create([proposal])
        proposal = Proposal.objects.get(code="DX-ASSIGN-005")
    ...
    def test_uses_latest_timesheet_entry_when_multiple_exist(self, db, test_employee):
        """Test that the latest TimeSheetEntry is used when multiple exist for the same date."""
        # Create two timesheet entries for the same date (edge case)
        first_entry = TimeSheetEntry.objects.create(
            employee=test_employee,
            date=date(2025, 11, 14),
        )
        second_entry = TimeSheetEntry.objects.create(
            employee=test_employee,
            date=date(2025, 11, 14),
        )
        ...
        proposal = Proposal(
            proposal_type=ProposalType.TIMESHEET_ENTRY_COMPLAINT,
            created_by=test_employee,
            timesheet_entry_complaint_complaint_date=date(2025, 11, 14),
            code="DX-ASSIGN-006",
        )
        Proposal.objects.bulk_create([proposal])
        proposal = Proposal.objects.get(code="DX-ASSIGN-006")

        proposal._assign_to_timesheet_entry()

        # Verify the latest entry (by id) was used
        junction = ProposalTimeSheetEntry.objects.get(proposal=proposal)
        assert junction.timesheet_entry == second_entry
...
class TestMeProposalVerifierAPI:
    """Tests for the me/proposal-verifiers/ endpoint.

    This endpoint returns proposal verifiers where the current user is the assigned employee.
    """

    def test_me_proposal_verifiers_returns_only_user_assignments(self, api_client, superuser, test_employee):
        """Test that me-proposal-verifiers returns only verifiers where user is the assigned employee."""
        # Link the superuser to the test employee
        test_employee.user = superuser
        test_employee.save()
        ...
        # Create another employee
        other_employee = Employee.objects.create(
            code="MV_OTHER_VERIFIER",
            fullname="Other Verifier",
            username="other_verifier_001",
            email="otherverifier001@example.com",
            phone="0977701001",
        ...
        ProposalVerifier.objects.create(proposal=proposal, employee=test_employee)

        # Add other_employee as verifier to another proposal
        proposal2 = Proposal.objects.create(
            code="DX_VERIFIER002",
            proposal_type=ProposalType.PAID_LEAVE,
            note="Another proposal",
            created_by=test_employee,
        )
        ...
        data = response.json()
        assert data["success"] is True
        # Should only return verifier where test_employee is the assigned employee
        assert data["data"]["count"] == 1
        assert data["data"]["results"][0]["employee"]["id"] == test_employee.id

    def test_me_proposal_verifiers_returns_empty_when_no_assignments(self, api_client, superuser, test_employee):
        """Test that me-proposal-verifiers returns empty list when user has no verifier assignments."""
        test_employee.user = superuser
        test_employee.save()

        # Create proposal without assigning test_employee as verifier
        other_employee = Employee.objects.create(
            code="MV_OTHER_EMP",
            fullname="Other Employee",
            username="other_emp_001",
            email="otheremp001@example.com",
            phone="0966601001",
    ...
    def test_me_proposal_verifiers_supports_filtering(self, api_client, superuser, test_employee):
        """Test that me-proposal-verifiers supports filtering."""
        test_employee.user = superuser
        test_employee.save()
        ...
        other_employee = Employee.objects.create(
            code="MV_FILTER_EMP",
            fullname="Filter Employee",
            username="filter_emp_001",
            email="filteremp001@example.com",
            phone="0955501001",
            attendance_code="55001",
            citizen_id="555000000001",
            branch=test_employee.branch,
            block=test_employee.block,
            department=test_employee.department,
            position=test_employee.position,
