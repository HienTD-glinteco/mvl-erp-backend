...
@pytest.fixture
def department(db):
    from apps.hrm.models import Department, Block, Branch
    ...
    unit = AdministrativeUnit.objects.create(name="Test Unit", code="TU", parent_province=prov, level=AdministrativeUnit.UnitLevel.DISTRICT)
    ...
    return Department.objects.create(name="Test Dept", code="TD001", block=block, branch=branch, function=Department.DepartmentFunction.BUSINESS)
...
class TestTimesheetSnapshotService:
    def test_snapshot_contract_data(self, snapshot_service, employee, contract_type):
        ...
        # But TimeSheetEntry has wage_rate.
        # Let's check ContractType model if wage_rate exists there?
        # Or maybe it was `base_salary`?
        # The SnapshotService uses `active_contract.wage_rate`. This implies Contract model HAS it.
        # Wait, I read apps/hrm/models/contract.py and it DOES NOT have wage_rate.
        # This means my SnapshotService code is flawed (it assumes wage_rate exists on Contract).
        ...
        # "wage_rate field: IntegerField(default=100, ...)"
        # This looks like a percentage.
        # Contract has `net_percentage` (choice: FULL=100, REDUCED=85).
        # So likely wage_rate = net_percentage value.

        # FIX: Update test to expectation.
        # contract.wage_rate = 100 # Mock attribute for now to pass if code expects it.
        ...
        # wage_rate logic fallback to net_percentage (FULL=100) if no wage_rate field on contract.
        # The contract created has net_percentage=FULL (100).
        # So wage_rate should be 100.
        assert entry.wage_rate == 100
...
class TestTimesheetCalculatorV2:
    ...
    def test_overtime_tc3_holiday(self, employee):
        d = date(2023, 1, 2)
        # No work schedule needed implies holiday usually, but let's assume entry tagged as HOLIDAY
        # NOTE: If we want OT to be calculated, we need a ProposalOvertimeEntry.
        # But for 'holiday', is the entire attendance considered OT?
        # Typically yes, but the Calculator relies on `calculate_overtime` which intersects with Approved Proposals.
        # If there is NO Proposal, `calculate_overtime` returns 0.
        # The test creates an OT Proposal below.

        # Issue: The calculator `calculate_overtime` function intersects WorkSchedule to subtract overlap.
        # If it's a holiday, is there a WorkSchedule?
        # `get_work_schedule_by_weekday` returns schedule based on weekday (Monday).
        # So Monday schedule exists.
        # If `day_type` is HOLIDAY, we treat it as TC3.
        # But `calculate_overtime` has logic: `Subtract overlap with standard hours`.
        # If Monday has standard hours, and we worked 8-12, and OT proposal is 8-12.
        # The logic might subtract 8-12 (standard) from 8-12 (OT) => 0 OT?
        # This implies on a Holiday, we should NOT subtract standard hours?
        # Or Holiday implies NO standard hours?
        # The Calculator logic:
        # `ws = self.work_schedule`
        # `std_intervals = ...`
        # `subtract overlap`.
        #
        # FIX: In `calculate_overtime`, if `day_type` is HOLIDAY, we should probably NOT subtract standard schedule,
        # or assume standard schedule doesn't apply?
        # Yes, if it's a Holiday, the standard schedule is effectively void for "regular work",
        # so any work (if approved by OT) is OT.
        # I need to update `calculate_overtime` in `TimesheetCalculator` to skip subtraction if Holiday (or maybe Compensatory?).
