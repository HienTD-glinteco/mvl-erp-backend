...
def _get_recruitment_reports_needing_refresh() -> tuple[date | None, list[tuple[int, int, int]]]:
    ...
    channel_date = RecruitmentChannelReport.objects.filter(need_refresh=True).aggregate(Min("report_date"))[
        "report_date__min"
    ]

    cost_date = RecruitmentCostReport.objects.filter(need_refresh=True).aggregate(Min("report_date"))[
        "report_date__min"
    ]

    hired_date = HiredCandidateReport.objects.filter(need_refresh=True).aggregate(Min("report_date"))[
        "report_date__min"
    ]
    ...
    for report in (
        RecruitmentCostReport.objects.filter(need_refresh=True, report_date__gte=earliest_date, report_date__lte=today)
        .values("branch_id", "block_id", "department_id")
        .distinct()
    ):
        org_units_set.add((report["branch_id"], report["block_id"], report["department_id"]))
    ...
    for report in (
        HiredCandidateReport.objects.filter(need_refresh=True, report_date__gte=earliest_date, report_date__lte=today)
        .values("branch_id", "block_id", "department_id")
        .distinct()
    ):
        org_units_set.add((report["branch_id"], report["block_id"], report["department_id"]))
...
def aggregate_recruitment_reports_batch() -> int:
    ...
    while current_date <= today:
        with transaction.atomic():
            ...
            RecruitmentSourceReport.objects.filter(report_date=current_date).update(need_refresh=False)

            RecruitmentChannelReport.objects.filter(report_date=current_date).update(need_refresh=False)

            RecruitmentCostReport.objects.filter(report_date=current_date).update(need_refresh=False)

            HiredCandidateReport.objects.filter(report_date=current_date).update(need_refresh=False)
