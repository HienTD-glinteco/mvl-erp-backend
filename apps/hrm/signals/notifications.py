...
from apps.hrm.constants import ProposalType, ProposalStatus, ProposalVerifierStatus
...
def _send_hr_notification(proposal, approved):
    ...
    # Scenario B: General Proposal
        ...
        if approved:
            message = _('Your %(proposal_type)s proposal has been approved by HR.') % {'proposal_type': proposal_type_display}
        else:
            note = proposal.approval_note or ""
            message = _('Your %(proposal_type)s proposal was rejected by HR with note: "%(note)s"..') % {'note': note, 'proposal_type': proposal_type_display}

    create_notification(
        actor=proposal.approved_by.user if proposal.approved_by and proposal.approved_by.user else recipient, # Fallback to recipient if no actor, but ideally should be system or HR user
        recipient=recipient,
...
def notify_proposal_verifier_change(sender, instance, created, **kwargs):
    ...
    if instance.status == ProposalVerifierStatus.VERIFIED:
        ...
        if proposal.proposal_type == ProposalType.TIMESHEET_ENTRY_COMPLAINT:
            message = _('Your timekeeping complaint has been confirmed by %(manager_name)s.') % {'manager_name': manager_name}
        else:
            proposal_type_display = proposal.get_proposal_type_display()
            message = _('Your %(proposal_type)s proposal has been confirmed by %(manager_name)s.') % {'proposal_type': proposal_type_display, 'manager_name': manager_name}
        ...
        # Rejected by Manager (Note: NOT_VERIFIED often acts as Rejection in some flows, let's assume so based on requirement "Rejected by Manager")
        # The requirement says "Rejected by Manager", usually mapping to a negative status.
        # In `ProposalVerifierStatus`: PENDING, NOT_VERIFIED, VERIFIED.
        # `NOT_VERIFIED` is likely the rejection status here given `ColorVariant.RED`.

        note = instance.note or ""

        if proposal.proposal_type == ProposalType.TIMESHEET_ENTRY_COMPLAINT:
            message = _('Your timekeeping complaint was rejected by %(manager_name)s with note: "%(note)s"..') % {'manager_name': manager_name, 'note': note}
        else:
            proposal_type_display = proposal.get_proposal_type_display()
            message = _('Your %(proposal_type)s proposal was rejected by %(manager_name)s with note: "%(note)s"..') % {'proposal_type': proposal_type_display, 'manager_name': manager_name, 'note': note}
