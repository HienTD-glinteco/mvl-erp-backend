...
def generate_recruitment_channel_code(sender, instance, created, **kwargs):
    """Auto-generate code for RecruitmentChannel when created.

    This signal handler generates a unique code for newly created RecruitmentChannel instances.
    It uses the instance ID to create a code in the format: {PREFIX}{subcode}

    Args:
        sender: The model class (RecruitmentChannel)
        instance: The RecruitmentChannel instance being saved
        created: Boolean indicating if this is a new instance
        **kwargs: Additional keyword arguments from the signal
