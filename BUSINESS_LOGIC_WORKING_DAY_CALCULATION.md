...
*   **Logic Tính OT (Hard-Rule):**
    ...
    *   *Lý do:* Không đủ dữ liệu đối soát. Yêu cầu nhân viên khiếu nại bổ sung công.

### 3.3. Xử lý Đi muộn / Về sớm (Penalty Logic)
**UC tham chiếu:** **UC 6.6.1.3**
Cần phân biệt rõ giữa việc **Tính công** (trừ lương theo giờ) và **Xác định lỗi** (phạt chuyên cần).
...
*   **Phân loại:**
    *   `ot_tc1_hours`: Ngày thường & Ngày làm bù.
    *   `ot_tc2_hours`: Ngày nghỉ tuần (T7/CN).
...
*   **Giới hạn:** `working_days` = Min(`working_days` + 0.125, `Max định mức`).
    *   *Ví dụ:* Ngày Thứ 7 (định mức 0.5), nhân viên làm 3 giờ thực tế (0.375) -> Cộng 1 giờ thành 4 giờ -> Đạt đủ **0.5 công**.
...
| :--- | :--- | :--- |
| **Lễ trùng ngày nghỉ** | **Công Lễ** | `day_type` = `holiday`. `working_days` = 1.0. |
...
| `is_full_salary` | Boolean | True nếu hưởng 100% lương (Mapping từ wage_rate). | `is_full_salary` |
| **Chi tiết Nghỉ & Lễ** | | | |
...
*   **`count_for_payroll`**:
    ...
    *   Nếu là nhân viên chính thức/thử việc có lương -> `True`.
    *   Nếu là CTV/Thực tập không lương -> `False`.

#### Nhóm 2: Field Giá trị Công (Core Value)
*   **`official_hours`**:
    *   `Sum(Morning_Hours + Afternoon_Hours)` (đã qua xử lý logic vào/ra ở Mục 3.1).
    *   Trừ đi `late_minutes` và `early_minutes` (quy đổi ra giờ).
*   **`working_days`**:
...
*   **`ot_tc1_hours`**:
    *   Overlap(Log, Đề xuất OT) nếu ngày là `official` (ngày thường) hoặc `compensatory`.
*   **`ot_tc2_hours`**:
    *   Overlap(Log, Đề xuất OT) nếu ngày là cuối tuần (T7/CN) và không phải ngày làm bù.
*   **`ot_tc3_hours`**:
...
1.  **Batch Job Hàng Tháng (`prepare_monthly_timesheets`):**
    ...
    *   **Hành động:** Tạo sẵn các record `TimeSheetEntry` cho từng ngày trong tháng. Tính toán ngay lập tức các giá trị Snapshot (contract_id, wage_rate, day_type) dựa trên dữ liệu hiện có.
