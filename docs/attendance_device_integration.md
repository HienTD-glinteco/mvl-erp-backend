...
- `apps.hrm.tasks.timesheets.prepare_monthly_timesheets` (cron: 1st of month at 00:01): prepares `TimeSheetEntry` rows and monthly aggregates for all active employees.
- `apps.hrm.tasks.timesheets.update_monthly_timesheet_async` (interval: 30s): processes `EmployeeMonthlyTimesheet` rows flagged with `need_refresh`.
 - `apps.hrm.tasks.timesheets.increment_available_leave_days` (cron: 1st of month at 00:02): increments `available_leave_days` for eligible employees by 1.
